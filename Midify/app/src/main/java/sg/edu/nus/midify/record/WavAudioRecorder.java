        ...
        try {
            if (state == State.INITIALIZING) {
                if ((audioRecorder.getState() == AudioRecord.STATE_INITIALIZED) & (filePath != null)) {
                    ...
                    Log.e(Constant.RECORD_TAG, "prepare() method called on uninitialized recorder");
                    state = State.ERROR;
                }
            } else {
                Log.e(Constant.RECORD_TAG, "prepare() method called on illegal state");
                release();
                state = State.ERROR;
            }
        } catch(Exception e) {
            if (e.getMessage() != null) {
                Log.e(Constant.RECORD_TAG, e.getMessage());
            } else {
                Log.e(Constant.RECORD_TAG, "Unknown error occured in prepare()");
            }
            state = State.ERROR;
        }
    }
        ...
        if (state == State.RECORDING) {
            ...
            if (state == State.READY){
                try {
                    ...
                    Log.e(Constant.RECORD_TAG, "I/O exception occured while closing output file");
                }
